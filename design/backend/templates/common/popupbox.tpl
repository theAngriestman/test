...
{elseif $act == "notes"}
    <a {$popup_params nofilter} class="cm-dialog-opener {$meta}"><span class="flex-inline top">{include_ext file="common/icon.tpl" class=$icon|default:'icon-question-sign'}</span></a>
{elseif $act == "general"}
        <div class="btn-group {$meta}">
            <a class="btn cm-dialog-opener {$link_class} cm-tooltip" {$popup_params nofilter} {if $edit_onclick}onclick="{$edit_onclick}"{/if} {if $href}href="{$href|fn_url}"{/if} {if $title}title="{$title}"{/if}>{if $icon}<span class="flex-inline top">{include_ext file="common/icon.tpl" class=$icon}</span>{/if} {$link_text}</a>
        </div>
{elseif $act == "button"}
