{if $is_image && $show_hidpi_checkbox|default:true}
    ...
    <label for="is_high_res_{$id_var_name}" class="hidpi-mark checkbox">
        ...
        {__("hidpi.upload_high_res_image")} <span class="flex-inline">{include_ext file="common/icon.tpl" class="icon-question-sign cm-tooltip" title=__("hidpi.upload_high_res_image.tooltip") icon_text=""}</span>
    </label>
{/if}
